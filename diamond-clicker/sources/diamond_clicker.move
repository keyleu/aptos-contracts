module diamond_clicker::game {
    ...
    */
    ...
    public fun initialize_game(account: &signer) {
        ...
        let empty_diamonds: vector<Upgrade> = vector::empty();
        let game_store = GameStore {
            diamonds: 0,
            upgrades: empty_diamonds,
    ...
    fun get_unclaimed_diamonds(account_address: address, current_timestamp_seconds: u64): u64 acquires GameStore {
        ...
        let i = 0;
        let end = vector::length(&game_store.upgrades);
        let minutes = (current_timestamp_seconds - game_store.last_claimed_timestamp_seconds) / 60;
        let unclaimed_diamonds = 0;
        loop {
            if(i >= end) break;
            let upgrade = vector::borrow(&game_store.upgrades, i);
            let (_found, index) = vector::index_of<vector<u8>>(&POWERUP_NAMES, &upgrade.name);
            let power_up = vector::borrow(&POWERUP_VALUES, index);
            let unclaimed = *vector::borrow(power_up, 1) * minutes * upgrade.amount;
            unclaimed_diamonds = unclaimed_diamonds + unclaimed;
            i = i + 1;
        };
    ...
    fun claim(account_address: address) acquires GameStore {
        ...
        let unclaimed_diamonds = get_unclaimed_diamonds(account_address, time_now);
        ...
        game_store.last_claimed_timestamp_seconds = time_now;
    }
    ...
    public fun get_diamonds_per_minute(account_address: address): u64 acquires GameStore {
        ...
        loop {
            ...
            let (_found, index) = vector::index_of<vector<u8>>(&POWERUP_NAMES, &upgrade.name);
            let power_up = vector::borrow(&POWERUP_VALUES, index);
            let upgrade_cost = *vector::borrow(power_up, 1);
            dpm = dpm + upgrade.amount * upgrade_cost;
            i = i + 1;
        };
